...
.gjamTrueVest <- function( pchains, true, typeCode, allTypes, xlim=NULL, ylim=NULL,
 ...
 # true   <- as.vector(true)
  ...
  #  bCoeffTable <- .processPars(pchains,xtrue=true )
...
.setupReductOld <- function(modelList, S, Q, n){
...
.getTimeIndex <- function(timeList, other, notOther, xdata, x, xl, y, w,
  ...
  if(length(timeZero) != length(timeLast))stop('timeZero and timeLast different lengths')
  ...
  if(length(w2) > 0)stop('some sequences have only 2 time intervals, need at least 3')
...
.gjam <- function(formula, xdata, ydata, modelList, verbose = FALSE){
  ...
  if( termR ){
    tmp <- .gjamMissingValues(xl, y, factorRho$factorList, typeNames, verbose)
...
.chain2tab <- function(chain, snames = NULL, xnn = NULL, wF = NULL, sigfig = 3){
 ...
 #   ww <- which(colSums(mmat, na.rm=T) != 0)
 #   mmat <- mmat[, ww, drop = FALSE]
 #   smat <- smat[, ww, drop = FALSE]
...
sqrtSeqNot <- function(xx, nbin = 10, power = .5){ #labels for sqrt scale
  
  # maxval on power scale
  ...
  mx <- mean(xx)
  sx <- sd(xx)
  skew <- ( mean(xx^3) - 3*mx*sx^2 - mx^3 )/sx^3
  power <- 1/skew
  if(abs(skew) < 1){
    power <- .5
    skew  <- 2
  ...
  maxval <- max( 1.01*xx^power )
  minval <- min( .99*xx^power )
  ...
  labs <- seq(minval^skew, maxval^skew, by = by)
  ...
  list(at = at, labs = labs, power = power)
...
.gjamPlot <- function(output, plotPars, verbose = FALSE){
  ...
  if(CHAINS){
    ...
    #  colR <- colorRampPalette(c('black','brown','orange'))
      cols <- colF(nrow(xt))
...
.gjamPrediction <- function(output, newdata, y2plot, PLOT, ylim, FULL, verbose = FALSE){
  ...
  #  xnew[,wss] <- t( (t(xnew[,wss]) - standX[wss,'xmean'])/   #CHECK THIS STANDARDIZATION
  #                     standX[wss,'xsd'])
...
.updateTheta <- function(w,tg,cutLo,cutHi,ordCols,holdoutN,
  ...
  maxw <- 2*max(w, na.rm=T) # will this drift???
 ...
 # cmax <- chi
  ...
  #  wi <- which(chi[,i] > clo[,i+1] & is.finite(clo[,i+1]))
  #  if(length(wi) > 0)chi[wi,i] <- (clo[wi,i] + clo[wi,i+1])/2
...
.wWrapperTime <- function(sampleW, y, timeZero, timeLast, i1, i2, tindex, gindex, uindex,
  ...
  function(w, plo, phi, wpropTime, xl, yp, Rmat, Amat, rndEff, groupRandEff, sdg, muw, mub,
   ...
   #   if(length(corCols) > 0){
   #     muw[,corCols]    <- .sqrtRootMatrix(muw[,corCols],sg[corCols,corCols],
   #                                         DIVIDE=T)
   #     muStar[,corCols] <- .sqrtRootMatrix(muStar[,corCols],sg[corCols,corCols],
   #                                         DIVIDE=T)
   #   }
   ...
   #     if(length(corCols) > 0){
   #       pnow[,corCols] <- dnorm( z0[,corCols], muw[i00,corCols],1,log=T) +
   #                         dnorm( z1[,corCols], muw[i11,corCols],1,log=T)
   #       pnew[,corCols] <- dnorm( Z0[,corCols], muw[i00,corCols],1,log=T) + 
   #                         dnorm( z1[,corCols], muStar[,corCols],1,log=T)
   #     }
    ...
    #   if(length(corCols) > 0){
    #      pnow[,corCols] <- .dMVN(z0[,corCols], muw[i00,corCols], smat = css[corCols,corCols],log=T) +
    #                        .dMVN(z1[,corCols], muw[i11,corCols], smat = css[corCols,corCols],log=T)
    #      pnew[,corCols] <- .dMVN(Z0[,corCols], muw[i00,corCols], smat = css[corCols,corCols],log=T) + 
    #                        .dMVN(z1[,corCols], muStar[,corCols], smat = css[corCols,corCols],log=T)
    #    }
   ...
   #   if(length(corCols) > 0){
   #     pnow[,corCols] <- dnorm( z1[,corCols], muw[timeLast,corCols],1,log=T)
   #     pnew[,corCols] <- dnorm( Z1[,corCols], muw[timeLast,corCols],1,log=T)
   #   }
  ...
  #    if(length(corCols) > 0){
  #      pnow[,corCols] <- .dMVN( z1[,corCols], muw[timeLast,corCols], smat = css[corCols,corCols],log=T)
  #      pnew[,corCols] <- .dMVN( Z1[,corCols], muw[timeLast,corCols], smat = css[corCols,corCols],log=T)
  #    }
  ...
  #    if(length(corCols) > 0){
  #      pnow[,corCols] <- .dMVN( z1[,corCols], muw[timeZero+1,corCols], smat = css[notOther,notOther], log=T)
  #      pnew[,corCols] <- .dMVN( z1[,corCols], muStar[,corCols], smat = css[notOther,notOther], log=T)
  #    }
  ...
  #    yp[,notOther] <- .rMVN(n, mu, sigma = sg[notOther,notOther])
